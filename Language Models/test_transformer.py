...
def load_saved_model(path, model_class, device):
    ...
    num_embeddings = hyperparams['embed']['n_embeddings']
    embedding_dim = hyperparams['embed']['embedding_dim']
...
if __name__ == "__main__":
    ...
    parser.add_argument("--path_to_transformer_model", type=str, required=True, help="Directory containing saved Transformer model.")
    ...
    transformer_model = load_saved_model(args.path_to_transformer_model, NWPTransformer, device)
    mappings = String2IntegerMapper.load(join(args.path_to_transformer_model, "w2i"))
    ...
    # Define output file for Transformer test results
    transformer_output_file = join(args.path_to_transformer_model, 'transformer_test_results.csv')

    # Test Transformer model and save results
    test_model(transformer_model, inputs_t, targets_t, processed_sentences, transformer_output_file, device, args.language)
