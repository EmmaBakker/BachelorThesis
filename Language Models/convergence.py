...
class ConvergenceState:
    def __init__(self):
        ...
        self.perplexity = None  # Add perplexity state

    def update(self, n_epoch, all_losses, output_probabilities, target_probabilities, training_accuracy, perplexity=None):
        self.all_losses = all_losses
        self.output_probabilities = output_probabilities
        self.target_probabilities = target_probabilities
        self.n_epoch = n_epoch
        self.training_accuracy = training_accuracy
        self.perplexity = perplexity  # Update perplexity state
...
class ConvergenceCriteria:
    ...
    def update_state(self, epochs, all_losses, output_probabilities, target_probabilities, training_accuracy, perplexity=None):
        self.state.update(epochs, all_losses, output_probabilities, target_probabilities, training_accuracy, perplexity)
        return self
...
class ConvergenceCriterion:
    def __init__(self, criterion_type, options):
        ...
        if criterion_type == "perplexity":
            self.max_epochs_stagnated = options["max_epochs_stagnated"]
            self.min_change = options["min_change"]
            self.__hasConverged = self.convergence_criterion_perplexity
    ...
    def __call__(self, state):
        ...
        elif self.criterion_type == "perplexity":
            return self.__hasConverged(state.perplexity)
        else:
    ...
    def convergence_criterion_perplexity(self, perplexities):
        """ Returns True if the model has stagnated in terms of perplexity. """
        if len(perplexities) < self.max_epochs_stagnated:
            return False

        prev = np.mean(npa(perplexities[-2]))
        act = np.mean(npa(perplexities[-1]))
        diff_proportion = 1 - act / prev
        if abs(diff_proportion) < self.min_change:
            self.__n_epochs_stagnant += 1
        else:
